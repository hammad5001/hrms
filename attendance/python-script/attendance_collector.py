...
class DailyAttendanceTracker:
    ...
    def ensure_tables_exist(self):
        """Create attendance_raw and employees tables if they don't exist"""
        try:
            ...
            conn.commit()
            print("✅ Tables 'attendance_raw' and 'employees' verified/created")
    ...
    def sync_users_to_mysql(self):
        """Sync users from device to MySQL employees table"""
        ...
        try:
            ...
            for _, row in self.users_df.iterrows():
                ...
                cursor.execute("SELECT id, full_name FROM employees WHERE employee_code = %s", (user_id,))
                ...
                if existing:
                    ...
                    if name != existing_name and not name.startswith('User_'):
                        cursor.execute(
                            "UPDATE employees SET full_name = %s WHERE employee_code = %s",
                            (name, user_id)
                        )
                        updated += 1
                    ...
                    # Insert new employee
                    cursor.execute(
                        "INSERT INTO employees (employee_code, full_name, department, is_active) VALUES (%s, %s, 'General', 1)",
                        (user_id, name)
                    )
                    added += 1
            ...
            conn.commit()
            print(f"\n✅ User sync complete: {added} new employees added, {updated} updated")
            
            # Get total count
            cursor.execute("SELECT COUNT(*) as count FROM employees WHERE is_active = 1")
            total = cursor.fetchone()[0]
            print(f"📊 Total active employees in database: {total}")
    ...
    def fetch_march_data(self):
        ...
        try:
            ...
            if not march_df.empty:
                march_df.to_csv("attendance_march_2026.csv", index=False)
                print("\n📁 March data saved to: attendance_march_2026.csv")

            if shift_summaries:
                summary_df = pd.DataFrame(shift_summaries)
                summary_df.to_csv("shifts_march_2026.csv", index=False)
                print("📊 Shift summary saved to: shifts_march_2026.csv")
                ...
                for date in sorted(date_groups.keys()):
                    ...
                    print(f"   {date}: {len(shifts)} employees (Present: {present_count}, Late: {late_count})")
    ...
    def fetch_april_data(self):
        ...
        try:
            ...
            if not april_df.empty:
                april_df.to_csv("attendance_april_2026.csv", index=False)
                print("\n📁 April data saved to: attendance_april_2026.csv")

            if shift_summaries:
                summary_df = pd.DataFrame(shift_summaries)
                summary_df.to_csv("shifts_april_2026.csv", index=False)
                print("📊 Shift summary saved to: shifts_april_2026.csv")
    ...
    def fetch_may_data(self):
        ...
        try:
            ...
            if not may_df.empty:
                may_df.to_csv("attendance_may_2026.csv", index=False)
                print("\n📁 May data saved to: attendance_may_2026.csv")

            if shift_summaries:
                summary_df = pd.DataFrame(shift_summaries)
                summary_df.to_csv("shifts_may_2026.csv", index=False)
                print("📊 Shift summary saved to: shifts_may_2026.csv")
        ...
        if choice == '1':
            ...
            if confirm in ('yes', 'y'):
                ...
                if success:
                    print("\n✅ Successfully fetched all March 2026 data!")
                    print("📁 Files created:")
                    print("   - attendance_march_2026.csv (all raw punches)")
                    print("   - shifts_march_2026.csv (shift summaries)")
                    print("   - Data also added to attendance_master.csv and MySQL")
                else:
            ...
            if confirm in ('yes', 'y'):
                ...
                if success:
                    print("\n✅ Successfully fetched all April 2026 data!")
                    print("📁 Files created:")
                    print("   - attendance_april_2026.csv (all raw punches)")
                    print("   - shifts_april_2026.csv (shift summaries)")
                    print("   - Data also added to attendance_master.csv and MySQL")
                else:
        ...
        # =====================================================
            ...
            if confirm in ('yes', 'y'):
                ...
                if success:
                    print("\n✅ Successfully fetched all May 2026 data!")
                    print("📁 Files created:")
                    print("   - attendance_may_2026.csv (all raw punches)")
                    print("   - shifts_may_2026.csv (shift summaries)")
                    print("   - Data also added to attendance_master.csv and MySQL")
                else:
